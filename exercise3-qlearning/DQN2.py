...
class Agent:
    def __init__(self, max_mem_size, action_space_size, state_size, gamma):
        self.old_policy = Policy(state_size,action_space_size)
        self.current_policy = Policy(state_size,action_space_size)
        self.memory = ReplayBuffer(max_mem_size)
        self.optimizer = torch.optim.Adam(self.current_policy.parameters(), lr=1e-3)
    ...
    def take_action(self, state, eps, train=True):
        ...
        with torch.no_grad():
            q_values = self.current_policy(torch.from_numpy(state))
        if train:
    ...
    def update_policy(self, batch_size):
        ...
        for transition in batch:
            ...
            with torch.no_grad():
                target = reward
                if done == False:
                    target += self.gamma*torch.max(self.old_policy(new_state))
            q_values = self.current_policy(state)
