...
def load_model(args, env):
    if args.algo == 'ppo':
        model = PPO.load("ppo_cartpole.mdl", env=env)
    elif args.algo == 'sac':
        model = SAC.load("sac_cartpole.mdl", env=env)
    else:
        raise ValueError(f"RL Algo not supported: {args.algo}")
    return model
...
if __name__ == "__main__":
    ...
    if args.test is None:
        try:
            ...
            # Policy training (T4) 
            model.learn(int(args.test_episodes))
            # Saving model (T4) 
            if args.algo == 'ppo':
                model.save("ppo_cartpole.mdl")
            elif args.algo == 'sac':
                model.save("sac_cartpole.mdl")
            else:
                raise ValueError(f"RL Algo not supported: {args.algo}")
